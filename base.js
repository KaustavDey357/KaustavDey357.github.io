/* base.js: helpers, icons, DotGrid, text effects */
  ...
  /* ---------- Aurora background ---------- */
  function Aurora() {
    return h('div', { className: 'aurora', 'aria-hidden': true }, h('span'), h('span'), h('span'));
  }
  ...
  function Rise(props) {
    var ref = useRef(null), seen = useInView(ref, props.threshold || 0.2);
    return h(props.as || 'div', {
      ref: ref, className: (props.className || '') + ' rise-el' + (seen ? ' in' : ''), style: props.style
    }, props.children);
  }

  function RiseGroup(props) {
    var ref = useRef(null), seen = useInView(ref, props.threshold || 0.15);
    return h(props.as || 'div', {
      ref: ref, className: (props.className || '') + ' rise-group' + (seen ? ' in' : ''), style: props.style
    }, props.children);
  }
  ...
  function Magnetic(props) {
    ...
    function move(e) {
      ...
      el.style.transform = 'translate(' + (mx * 0.16).toFixed(1) + 'px,' + (my * 0.26).toFixed(1) + 'px)';
    }
    function leave() { if (ref.current) ref.current.style.transform = ''; }
    return h('span', { className: 'magnet', ref: ref, onMouseMove: move, onMouseLeave: leave }, props.children);
  }
  ...
  function ScrollProgress() {
    ...
    return h('div', { className: 'scroll-progress', 'aria-hidden': true }, h('div', { className: 'scroll-progress-bar', ref: ref }));
  }
